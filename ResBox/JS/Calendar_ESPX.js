function CalendarLoad(B, D, C, A) {
    Calendar.setup({inputField: B, ifFormat: "%d-%m-%Y", button: D, dateStatusFunc: disallowDate, range: [C, A]})
}
function FechaGet(E, D, G, C) {
    var B = E.value;
    var A = B.split("-");
    var F = new Date();
    var H = new Date(A[2], A[1] - 1, A[0], 10, 0, 0);
    F.setHours(0);
    if (H >= F) {
        D.value = A[2];
        if (A[1].substr(0, 1) == "0") {
            G.value = A[1].substr(1, 1)
        } else {
            G.value = A[1]
        }
        fillMonthsSelect(D.value, G, C, false);
        if (A[0].substr(0, 1) == "0") {
            C.value = A[0].substr(1, 1)
        } else {
            C.value = A[0]
        }
    }
}
function loadCalendar(B, A) {
    Calendar.setup({inputField: "fecha", ifFormat: "%d-%m-%Y", button: "Cal", dateStatusFunc: disallowDate, range: [B, A]})
}
function getFecha() {
    var B = document.getElementById("fecha").value;
    var A = B.split("-");
    var C = new Date();
    var D = new Date(A[2], A[1] - 1, A[0], 10, 0, 0);
    C.setHours(0);
    if (D >= C) {
        document.getElementById("anio").value = A[2];
        if (A[1].substr(0, 1) == "0") {
            document.getElementById("mes").value = A[1].substr(1, 1)
        } else {
            document.getElementById("mes").value = A[1]
        }
        fillMonthsSelect(document.getElementById("anio").value, document.getElementById("mes"), document.getElementById("dia"), false);
        if (A[0].substr(0, 1) == "0") {
            document.getElementById("dia").value = A[0].substr(1, 1)
        } else {
            document.getElementById("dia").value = A[0]
        }
    }
}
var meses = new Array("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec");
function ActualizaInput() {
    var B = document.getElementById("HiddenFrom").value;
    var A = B.split("/");
    var D = new Date();
    var C = meses[parseInt(A[0]) - 1];
    if (A[0].substr(0, 1) == "0") {
        C = meses[A[0].substr(1, 1) - 1]
    }
    var E = C + " " + A[1] + ", " + A[2];
    D.setDate(D.getDate() - 1);
    if (Date.parse(E) >= Date.parse(D)) {
        document.getElementById("Departure").value = B
    }
}
function loadCalendarFrom(B, A) {
    Calendar.setup({inputField: "fechaFrom", ifFormat: "%d-%m-%Y", button: "CalFrom", dateStatusFunc: disallowDate, range: [B, A]})
}
function loadCalendarTo(B, A) {
    Calendar.setup({inputField: "fechaTo", ifFormat: "%d-%m-%Y", button: "CalTo", dateStatusFunc: disallowDate, range: [B, A]})
}
function getFechaFrom() {
    var B = document.getElementById("fechaFrom").value;
    var A = B.split("-");
    var C = new Date();
    var D = new Date(A[2], A[1] - 1, A[0], 10, 0, 0);
    C.setHours(0);
    if (D >= C) {
        document.getElementById("anio_desde").value = A[2];
        if (A[1].substr(0, 1) == "0") {
            document.getElementById("mes_desde").value = A[1].substr(1, 1)
        } else {
            document.getElementById("mes_desde").value = A[1]
        }
        fillMonthsSelect(document.getElementById("anio_desde").value, document.getElementById("mes_desde"), document.getElementById("dia_desde"), false);
        if (A[0].substr(0, 1) == "0") {
            document.getElementById("dia_desde").value = A[0].substr(1, 1)
        } else {
            document.getElementById("dia_desde").value = A[0]
        }
    }
}
function getFechaTo() {
    var B = document.getElementById("fechaTo").value;
    var A = B.split("-");
    var C = new Date();
    var D = new Date(A[2], A[1] - 1, A[0], 10, 0, 0);
    C.setHours(0);
    if (D >= C) {
        document.getElementById("anio_hasta").value = A[2];
        if (A[1].substr(0, 1) == "0") {
            document.getElementById("mes_hasta").value = A[1].substr(1, 1)
        } else {
            document.getElementById("mes_hasta").value = A[1]
        }
        fillMonthsSelect(document.getElementById("anio_hasta").value, document.getElementById("mes_hasta"), document.getElementById("dia_hasta"), false);
        if (A[0].substr(0, 1) == "0") {
            document.getElementById("dia_hasta").value = A[0].substr(1, 1)
        } else {
            document.getElementById("dia_hasta").value = A[0]
        }
    }
}
...
Calendar.prototype._init = function(L, V) {
    var U = new Date(), P = U.getFullYear(), X = U.getMonth(), B = U.getDate();
...
Calendar.setup = function(G) {
    ...
    D["on" + G.eventName] = function() {
        ...
        if (!(L && G.cache)) {
            window.calendar = L = new Calendar(G.firstDay, G.date, G.onSelect || A, G.onClose || function(O) {
                O.hide()
            });
...
Calendar._DN = new Array("Domingo", "Lunes", "Martes", "Miercoles", "Jueves", "Viernes", "Sabado", "Domingo");
...
Calendar._TT.ABOUT = "      Selecci�n de fecha:\n      - Use los botones \xab, \xbb para seleccionar el a�o\n      - Use los botones " + String.fromCharCode(8249) + ", " + String.fromCharCode(8250) + " para seleccionar el mes\n      - Mantenga oprimido el bot�n del mouse en cualquiera de los botones superiores para una selecci�n r�pida.";
Calendar._TT.ABOUT_TIME = "\n\nTime selection:\n- Click on any of the time parts to increase it\n- or Shift-click to decrease it\n- or click and drag for faster selection.";
Calendar._TT.PREV_YEAR = "A�o previo (sostener para menu)";
Calendar._TT.PREV_MONTH = "Mes previo (sostener para menu)";
Calendar._TT.GO_TODAY = "Hoy";
Calendar._TT.NEXT_MONTH = "Mes siguiente (sostener para menu)";
Calendar._TT.NEXT_YEAR = "A�o siguiente (sostener para menu)";
...
function CambiaManual() {
    document.getElementById("fecha").value = document.getElementById("dia").value + "-" + document.getElementById("mes").value + "-" + document.getElementById("anio").value
}
function CambiaFromManual() {
    document.getElementById("fechaFrom").value = document.getElementById("dia_desde").value + "-" + document.getElementById("mes_desde").value + "-" + document.getElementById("anio_desde").value
}
function CambiaToManual() {
    document.getElementById("fechaTo").value = document.getElementById("dia_hasta").value + "-" + document.getElementById("mes_hasta").value + "-" + document.getElementById("anio_hasta").value
}
function ManualCambia(C, B, D, A) {
